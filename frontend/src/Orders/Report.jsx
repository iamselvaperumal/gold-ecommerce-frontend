...
export default function Report() {
  ...
  const handleExportExcel = () => {
    // ── Sheet 1: Summary ──
    const summaryData = [
      ['Report Name', `Sales Report - ${cfg.label}`],
    ...
    // ── Sheet 2: Network breakdown (table data) ──
    const tableHeader = [...columns, 'Orders', 'Sales']
    const tableRows = allRows.map(row => [...row.chain, row.orders, row.amount])
    const detailSheet = XLSX.utils.aoa_to_sheet([tableHeader, ...tableRows])
    ...
    // ── Download ──
    const fileName = `Sales_Report_${role}_${new Date().toISOString().slice(0, 10)}.xlsx`
