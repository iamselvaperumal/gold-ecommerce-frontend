...
export default function DiamondBracelets() {
  ...
  useEffect(() => {
    import('../api').then(({ default: api }) => {
      api.get('/jewelry-products/?category=bracelets&metal=diamond')
      ...
      <div style={{ padding: '40px', maxWidth: '1400px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '20px' }}>
          {loading ? (
            ...
            <div style={{ gridColumn: 'span 3', textAlign: 'center', color: '#64748b', padding: '60px 0' }}>No diamond bracelets found.</div>
            ...
            return (
              <div key={product.id} className="dm-card"
                onClick={() => navigate(`/product-display?category=bracelets&metal=diamond&id=${product.id}`)}
                ...
                <div className="dm-img-wrap" style={{ position: 'relative', height: '240px', background: '#f8fafc' }}>
                  ...
                  {product.grade && (
                    <div style={{ position: 'absolute', top: '10px', left: '10px', background: 'rgba(103,232,249,0.2)', border: '1px solid rgba(103,232,249,0.5)', borderRadius: '16px', padding: '3px 10px', color: ACCENT, fontSize: '9px', fontWeight: 800, backdropFilter: 'blur(8px)' }}>
