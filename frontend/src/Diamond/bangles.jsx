...
const TAG_COLORS = {
  ...
  Limited: { bg: 'rgba(103,232,249,0.15)', border: 'rgba(103,232,249,0.4)', color: '#22d3ee' },
}

const DIAMOND_COLOR = '#67e8f9'
const DIAMOND_GLOW = 'rgba(103,232,249,0.28)'

export default function DiamondBangles() {
  const navigate = useNavigate()
  const [products, setProducts] = useState([])
  const [loading, setLoading] = useState(true)
  const [hovered, setHovered] = useState(null)
  const [wishlistedIds, setWishlistedIds] = useState(new Set())
  const [gradeFilter, setGradeFilter] = useState('all') // 'all' | '18k' | '22k'
  ...
  const tagStyle = (tag) => TAG_COLORS[tag] || { bg: 'rgba(103,232,249,0.1)', border: 'rgba(103,232,249,0.3)', color: DIAMOND_COLOR }

  useEffect(() => {
    import('../api').then(({ default: api }) => {
      api.get('/jewelry-products/?category=bangles&metal=diamond')
  ...
  const filteredProducts = gradeFilter === 'all'
    ? products
      ...
      <div style={{ padding: '40px', maxWidth: '1400px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '20px' }}>
          {loading ? (
            ...
            <div style={{ gridColumn: 'span 3', textAlign: 'center', color: '#64748b', padding: '60px 0' }}>No diamond bangles found.</div>
            ...
            return (
              <div key={product.id} className="dm-card"
                onClick={() => navigate(`/product-display?category=bangles&metal=diamond&id=${product.id}`)}
                onMouseEnter={() => setHovered(product.id)}
                onMouseLeave={() => setHovered(null)}
                style={{
                  borderRadius: '20px', overflow: 'hidden', cursor: 'pointer', position: 'relative',
                  border: `1px solid ${isHovered ? 'rgba(103,232,249,0.55)' : 'rgba(103,232,249,0.18)'}`,
                  background: isHovered ? 'rgba(103,232,249,0.05)' : '#fff',
                  transform: isHovered ? 'translateY(-10px) scale(1.02)' : 'translateY(0) scale(1)',
                  boxShadow: isHovered ? `0 20px 50px ${DIAMOND_GLOW}` : '0 2px 12px rgba(0,0,0,0.06)',
                  transition: 'all 0.35s cubic-bezier(0.34,1.56,0.64,1)',
                }}>
                <div className="dm-shine" />
                <div className="dm-img-wrap" style={{ position: 'relative', height: '240px', background: '#f0f9ff' }}>
                  {imgUrl ? (
                  ...
                  {/* Grade badge */}
                  {product.grade && (
                    <div style={{ position: 'absolute', top: '10px', left: '10px', background: 'rgba(103,232,249,0.2)', border: '1px solid rgba(103,232,249,0.5)', borderRadius: '16px', padding: '3px 10px', color: DIAMOND_COLOR, fontSize: '9px', fontWeight: 800, backdropFilter: 'blur(8px)' }}>
                ...
                <div style={{ padding: '14px 16px' }}>
                  <div style={{ color: isHovered ? DIAMOND_COLOR : '#020617', fontWeight: 800, fontSize: '14px', marginBottom: '4px', transition: 'color 0.3s' }}>{product.name}</div>
                  {product.description && (
                    <div style={{ color: '#64748b', fontSize: '11px', lineHeight: '1.5', marginBottom: '8px' }}>{product.description}</div>
                  )}
                  {product.price && (
                    <div style={{ color: DIAMOND_COLOR, fontWeight: 900, fontSize: '15px', fontFamily: 'monospace' }}>
                ...
                {isHovered && (
                  <div style={{ padding: '0 16px 14px', animation: 'fadeInUp 0.2s ease' }}>
                    <div style={{ width: '100%', padding: '8px', background: `linear-gradient(90deg,${DIAMOND_COLOR},#a5f3fc)`, borderRadius: '10px', color: '#000', fontWeight: 800, fontSize: '11px', textAlign: 'center' }}>
                      👁 View Details
                    </div>
                  </div>
                )}
