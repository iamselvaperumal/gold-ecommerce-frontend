...
class HierarchyChildrenView(APIView):
    ...
    def get(self, request):
        ...
        def node_order_count(user_ids):
            if not user_ids:
                return {}
            return dict(
                JewelryOrder.objects.filter(
                    user_id__in=user_ids, created_at__year=now.year, created_at__month=now.month
                ).values('user_id').annotate(c=Count('id')).values_list('user_id', 'c')
            )

        if role == 'admin':
            qs = AdminProfile.objects.all()
            data = [{
                'id': a.id, 'user_id': a.user_id, 'admin_id': a.admin_id,
                'first_name': a.first_name, 'last_name': a.last_name,
                'mobile_number': a.mobile_number, 'city_name': a.city_name,
                'dealer_count': a.assigned_dealers.count(),
            } for a in qs]

        elif role == 'dealer':
            qs = DealerProfile.objects.filter(assigned_admin_id=parent_id)
            data = [{
                'id': d.id, 'user_id': d.user_id, 'dealer_id': d.dealer_id,
                'first_name': d.first_name, 'last_name': d.last_name,
                'mobile_number': d.mobile_number, 'city_name': d.city_name,
                'sub_dealer_count': d.assigned_sub_dealers.count(),
            } for d in qs]

        elif role == 'sub_dealer':
            qs = SubDealerProfile.objects.filter(assigned_dealer_id=parent_id)
            data = [{
                'id': s.id, 'user_id': s.user_id, 'sub_dealer_id': s.sub_dealer_id,
                'first_name': s.first_name, 'last_name': s.last_name,
                'mobile_number': s.mobile_number, 'city_name': s.city_name,
                'promotor_count': s.assigned_promotors.count(),
            } for s in qs]

        elif role == 'promotor':
            qs = PromotorProfile.objects.filter(assigned_sub_dealer_id=parent_id)
            user_ids = [p.user_id for p in qs]
            counts = node_order_count(user_ids)
            data = [{
                'id': p.id, 'user_id': p.user_id, 'promotor_id': p.promotor_id,
                'first_name': p.first_name, 'last_name': p.last_name,
                'mobile_number': p.mobile_number, 'city_name': p.city_name,
                'customer_count': p.assigned_customers.count(),
                'order_count': counts.get(p.user_id, 0),
                'status': get_target_status(counts.get(p.user_id, 0)),
            } for p in qs]

        elif role == 'customer':
            qs = CustomerProfile.objects.filter(assigned_promotor_id=parent_id)
            ...
            data = [{
                ...
                'first_name': c.first_name, 'last_name': c.last_name,
                'mobile_number': c.mobile_number, 'city_name': c.city_name,
                'order_count': counts.get(c.user_id, 0),
                'status': get_target_status(counts.get(c.user_id, 0)),
            } for c in qs]
        else:
            return Response({'error': 'invalid role'}, status=400)
...
class HierarchySearchView(APIView):
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request):
        ...
        admins = AdminProfile.objects.filter(
            Q(admin_id__icontains=q) | Q(first_name__icontains=q) |
            Q(last_name__icontains=q) | Q(mobile_number__icontains=q)
        )[:10]
        for a in admins:
            results.append({
                'role': 'admin', 'id': a.id, 'user_id': a.user_id, 'admin_id': a.admin_id,
                'first_name': a.first_name, 'last_name': a.last_name,
                'mobile_number': a.mobile_number, 'city_name': a.city_name,
                'ancestors': [],
            })

        dealers = DealerProfile.objects.select_related('assigned_admin').filter(
            Q(dealer_id__icontains=q) | Q(first_name__icontains=q) |
            Q(last_name__icontains=q) | Q(mobile_number__icontains=q)
        )[:10]
        for d in dealers:
            a = d.assigned_admin
            results.append({
                'role': 'dealer', 'id': d.id, 'user_id': d.user_id, 'dealer_id': d.dealer_id,
                'first_name': d.first_name, 'last_name': d.last_name,
                'mobile_number': d.mobile_number, 'city_name': d.city_name,
                'ancestors': [{'role': 'admin', 'node': {
                    'id': a.id, 'admin_id': a.admin_id, 'first_name': a.first_name,
                    'last_name': a.last_name, 'mobile_number': a.mobile_number, 'city_name': a.city_name,
                }}] if a else [],
            })

        sub_dealers = SubDealerProfile.objects.select_related('assigned_dealer__assigned_admin').filter(
            Q(sub_dealer_id__icontains=q) | Q(first_name__icontains=q) |
            Q(last_name__icontains=q) | Q(mobile_number__icontains=q)
        )[:10]
        for sd in sub_dealers:
            d = sd.assigned_dealer
            a = d.assigned_admin if d else None
            anc = []
            if a:
                anc.append({'role': 'admin', 'node': {
                    'id': a.id, 'admin_id': a.admin_id, 'first_name': a.first_name,
                    'last_name': a.last_name, 'mobile_number': a.mobile_number, 'city_name': a.city_name,
                }})
            if d:
                anc.append({'role': 'dealer', 'node': {
                    'id': d.id, 'dealer_id': d.dealer_id, 'first_name': d.first_name,
                    'last_name': d.last_name, 'mobile_number': d.mobile_number, 'city_name': d.city_name,
                }})
            results.append({
                'role': 'sub_dealer', 'id': sd.id, 'user_id': sd.user_id, 'sub_dealer_id': sd.sub_dealer_id,
                'first_name': sd.first_name, 'last_name': sd.last_name,
                'mobile_number': sd.mobile_number, 'city_name': sd.city_name,
                'ancestors': anc,
            })
        ...
        promotors = PromotorProfile.objects.select_related(
            ...
            Q(promotor_id__icontains=q) | Q(first_name__icontains=q) |
            Q(last_name__icontains=q) | Q(mobile_number__icontains=q)
        )[:10]
        for p in promotors:
            sd = p.assigned_sub_dealer
            d = sd.assigned_dealer if sd else None
            a = d.assigned_admin if d else None
            anc = []
            if a:
                anc.append({'role': 'admin', 'node': {
            ...
            if sd:
                anc.append({'role': 'sub_dealer', 'node': {
                    'id': sd.id, 'sub_dealer_id': sd.sub_dealer_id, 'first_name': sd.first_name,
                    'last_name': sd.last_name, 'mobile_number': sd.mobile_number, 'city_name': sd.city_name,
                }})
            results.append({
                'role': 'promotor', 'id': p.id, 'user_id': p.user_id, 'promotor_id': p.promotor_id,
                'first_name': p.first_name, 'last_name': p.last_name,
                'mobile_number': p.mobile_number, 'city_name': p.city_name,
                'ancestors': anc,
            })
        ...
        customers = CustomerProfile.objects.select_related(
            'assigned_promotor__assigned_sub_dealer__assigned_dealer__assigned_admin'
        ).filter(
            Q(customer_id__icontains=q) | Q(first_name__icontains=q) |
            Q(last_name__icontains=q) | Q(mobile_number__icontains=q)
        )[:10]
        for c in customers:
            p = c.assigned_promotor
            sd = p.assigned_sub_dealer if p else None
            d = sd.assigned_dealer if sd else None
            a = d.assigned_admin if d else None
            anc = []
            if a:
            ...
            if p:
                anc.append({'role': 'promotor', 'node': {
                    'id': p.id, 'promotor_id': p.promotor_id, 'first_name': p.first_name,
                    'last_name': p.last_name, 'mobile_number': p.mobile_number, 'city_name': p.city_name,
                }})
            results.append({
                'role': 'customer', 'id': c.id, 'user_id': c.user_id, 'customer_id': c.customer_id,
                'first_name': c.first_name, 'last_name': c.last_name,
                'mobile_number': c.mobile_number, 'city_name': c.city_name,
                'ancestors': anc,
            })
