...
class Command(BaseCommand):
    help = 'Create dummy Dealer users, evenly distributed across all Admins'

    def add_arguments(self, parser):
        parser.add_argument('--count', type=int, default=200, help='Total number of dummy dealers to create')
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f"Found {total_admins} admins. Distributing {count} dealers among them..."))

        for i in range(1, count + 1):
            # ── Round-robin: dealer i goes to admin (i % total_admins) ──
            admin = admins[(i - 1) % total_admins]
            ...
            email = f"dummydealer{i}@bitbyte.test"
            ...
            self.stdout.write(self.style.SUCCESS(
                f"Created: {email} -> assigned to Admin {admin.admin_id} ({admin.first_name})"
            ))

        self.stdout.write(self.style.SUCCESS(f"\nDone! {created} dummy dealers created and linked to {total_admins} admins."))